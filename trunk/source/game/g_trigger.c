...
void deathballtarget_use (edict_t *self, edict_t *other, edict_t *activator)
{
	if (self->solid == SOLID_NOT)
		self->solid = SOLID_TRIGGER;
	else
...
void deathballtarget_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if(strcmp(other->classname, "item_deathball") == 0) {
		...
		if (((int)(dmflags->value) & DF_SKINTEAMS)) {
			...
			else if(other->owner->dmteam == BLUE_TEAM && strcmp(self->classname, "trigger_reddeathballtarget") == 0) {
				blue_team_score+=10;
				other->owner->client->resp.score += 10;
			}
			else
				return; //shot at wrong goal in a team game, loser!
		}
...
void SP_trigger_deathballtarget (edict_t *self)
{
	InitTrigger (self);
	...
	if (self->spawnflags & 2)
		self->use = deathballtarget_use;

	gi.linkentity (self);
}

void SP_trigger_reddeathballtarget (edict_t *self)
{
	InitTrigger (self);
...
void SP_trigger_bluedeathballtarget (edict_t *self)
{
	InitTrigger (self);
