    ...
    if(( currententity->flags & ( RF_SHELL_RED | RF_SHELL_GREEN | RF_SHELL_BLUE | RF_SHELL_DOUBLE | RF_SHELL_HALF_DAM) ) )
    {
        ...
        while (1)
        {
            ...
            do
            {
                ...
                if(lerped) {
                    VArray[0] = currententity->s_lerped[index_xyz][0] = move[0] + ov[index_xyz].v[0]*backv[0] + v[index_xyz].v[0]*frontv[0] + r_avertexnormals[verts[index_xyz].lightnormalindex][0] * POWERSUIT_SCALE * shellscale;;
                    VArray[1] = currententity->s_lerped[index_xyz][1] = move[1] + ov[index_xyz].v[1]*backv[1] + v[index_xyz].v[1]*frontv[1] + r_avertexnormals[verts[index_xyz].lightnormalindex][1] * POWERSUIT_SCALE * shellscale;;
                    VArray[2] = currententity->s_lerped[index_xyz][2] = move[2] + ov[index_xyz].v[2]*backv[2] + v[index_xyz].v[2]*frontv[2] + r_avertexnormals[verts[index_xyz].lightnormalindex][2] * POWERSUIT_SCALE * shellscale;;

                    VArray[3] = (currententity->s_lerped[index_xyz][1] + currententity->s_lerped[index_xyz][0]) * (1.0f / 40.0f);
                    VArray[4] = currententity->s_lerped[index_xyz][2] * (1.0f / 40.0f) - r_newrefdef.time * 0.5f;

                    VArray[5] = shadelight[0];
                    VArray[6] = shadelight[1];
                    VArray[7] = shadelight[2];
                    VArray[8] = calcEntAlpha(alpha, currententity->s_lerped[index_xyz]);
                }
                else {
        ...
        while (1)
        {
            ...
            do
            {
                ...
                if(lerped) {
                    ...
                    VArray[0] = currententity->s_lerped[index_xyz][0] = move[0] + ov[index_xyz].v[0]*backv[0] + v[index_xyz].v[0]*frontv[0];
                    VArray[1] = currententity->s_lerped[index_xyz][1] = move[1] + ov[index_xyz].v[1]*backv[1] + v[index_xyz].v[1]*frontv[1];
                    VArray[2] = currententity->s_lerped[index_xyz][2] = move[2] + ov[index_xyz].v[2]*backv[2] + v[index_xyz].v[2]*frontv[2];
            ...
            while (stage)
            {
                ...
                do
                {
                    ...
                    if (stage->envmap)
                    {
                        ...
                        VectorAdd(currententity->origin, currententity->s_lerped[index_xyz], envmapvec);
...

                    {
                        ...
                        if(lerped)
                            nAlpha = RS_AlphaFuncAlias (stage->alphafunc,
                                calcEntAlpha(alpha, currententity->s_lerped[index_xyz]), normal, currententity->s_lerped[index_xyz]);
                        else
                            nAlpha = RS_AlphaFuncAlias (stage->alphafunc,
...
void R_DrawAliasShadowLegacy(dmdl_t *paliashdr, qboolean lerped)
{
    ...
    while (1)
    {
        ...
        do
        {
            ...
            if(lerped)
                memcpy( point, currententity->s_lerped[order[2]], sizeof( point )  );
            else
...
void R_DrawAliasShadow(dmdl_t *paliashdr, qboolean lerped)
{
	...
	for (i=0; i<paliashdr->num_tris; i++)
	{
		...
		{			
			...
			if(lerped)
				memcpy( point, currententity->s_lerped[index_xyz], sizeof( point )  );
			else
