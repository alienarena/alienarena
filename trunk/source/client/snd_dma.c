...
void S_Init (void)
{
	...
	else
	{
		...
		s_khz = Cvar_Get ("s_khz", "22", CVAR_ARCHIVE); //default sound sampling rate will be 22000 in windows
...
void S_StartSound(vec3_t origin, int entnum, int entchannel, sfx_t *sfx, float fvol, float attenuation, float timeofs)
{
	...
	//hack - don't play music files from target speakers anymore
...
void S_AddLoopSounds (void)
{
	...
	for (i=0 ; i<cl.frame.num_entities ; i++)
	{
		...
		//hack - don't play music files from target speakers anymore
		if((sfx->name[0] == 'm' && sfx->name[1] == 'u') || (sfx->name[6] == 'm' && sfx->name[7] == 'a' && sfx->name[8] == 'r')
			|| (sfx->name[6] == 'd' && sfx->name[7] == 'a' && sfx->name[8] == 'l') 
			|| (sfx->name[6] == 't' && sfx->name[7] == 'o' && sfx->name[8] == 'k'))
			continue;
