...
'crded' to 'crx-ded'. (With the 7.60 release, updated to 'alienarena-ded').
...
install-exec' to limit copying to the executables. Note that
the alternate install always just copies the executables.
...
We should support separate source and data tarballs.  This would
make us more popular with distro packagers. I think they have to do this
split because the source and data are under different licenses.

=== Adding new source files ===

New source files are added to the 'source/Makefile.am' file. Add the
new source to the applicable '*_SOURCE' lists. Header ('.h') files must
be included, because the list is used to generate the tarball.

=== Maintainer's Reconfigure ===

TODO

=== Adding a system library to the build ===

For common system libraries, there will usually be an .m4 macro available.
These may be part of autoconf (AC_*), or they may be extensions from
the GNU Autoconf Archive (AX_*).

After adding new AX_ macros to configure.ac, the .m4 file needs to be added to
the m4 subdirectory. This is done with the following command.
...
Wextra::
  ...
  Use the '--enable-full-warnings' 'configure' option to turn these on.
...
The 'autoconf-archive' is an evolving set of macros maintained 
on the GNU website. (On gentoo it is installed in '/usr/share/aclocal').
We use a few of these. Each file has a 'serial' number in a comment. Care
should be take not to commit an earlier version of a '.m4' into the SVN 'm4'
...
the generated 'aclocal.m4' file. See below for commands to update the
macro files.
...
program, 'alienarena'. Only files that are used in the library build
